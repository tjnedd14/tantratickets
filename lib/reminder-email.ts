...
export async function sendReminderEmail(params: SendBlastParams) {
  ...
  const headline = isPromoMode
    ? (params.customHeadline?.trim() || params.customSubject!.trim())
    : params.isOpenBar
    ? "YOUR OPEN BAR<br/>STARTS AT 9PM"
    : "SEE YOU<br/>TONIGHT";

  const eyebrow = isPromoMode
    ? "TANTRA NIGHT CLUB"
    : `TONIGHT · ${dateStr.toUpperCase()}`;
    ...
    ? `
      <tr>
        <td style="padding: 30px 30px 10px; color:#fff;">
          <p style="font-size:18px; font-weight:bold; margin:0 0 12px;">Hey ${firstName},</p>
          ${
            params.customMessage
              ? `<p style="font-size:15px; line-height:1.6; color:#ddd; margin:0; white-space:pre-wrap;">${escapeHtml(params.customMessage)}</p>`
              : ""
          }
        </td>
      </tr>
    ...
    : `
      <tr>
        <td style="padding: 30px 30px 10px; color:#fff;">
          ...
          ${params.customMessage ? `<p style="font-size:15px; line-height:1.5; color:#fff; margin:16px 0 0; padding:16px; background:#1a1a1a; border-left:3px solid #DB130D;">${escapeHtml(params.customMessage)}</p>` : ""}
        </td>
      </tr>
    `;
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="560" style="max-width:560px; background:#0E0E0E; border: 1px solid #1a1a1a;">
          ...
          <tr>
            <td align="center" style="padding: 0 30px 30px;">
              ...
              <div style="font-size:${isPromoMode ? "26" : "32"}px; font-weight:900; letter-spacing:-1px; line-height:1.15; color:#fff;">
                ${headline}
              </div>
